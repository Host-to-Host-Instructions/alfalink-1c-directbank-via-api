...
| 2    | POST  | /API/v1/directbank/SendPack    | отправка пакета документов                       | Идентификатор сессии<br />Пакет с запросом выписки/платежом/ведомостью | Идентификатор запроса (дальше не используется)/Ошибка (если запрос/пакет некорректный) |
